...
namespace mini
{
	...
	public:
		...
		template<typename InputIt/*, typename std::enable_if<std::is_default_constructible<deleter_type>::value && std::is_constructible<value_type, decltype(*declval<InputIt>())>::value, int>::type = 0*/>
		...
		template<typename InputIt/*, typename std::enable_if<std::is_constructible<value_type, decltype(*declval<InputIt>())>::value, int>::type = 0*/>
		...
		/*template<typename InputIt, typename std::enable_if<std::is_default_constructible<deleter_type>::value && std::is_same<unique_ptr_type&&, decltype(*declval<InputIt>())>::value, int>::type = 0>
		ptr_vector(InputIt first, InputIt last, const allocator_type& alloc = allocator_type{})
		...
		template<typename InputIt, typename std::enable_if<std::is_same<unique_ptr_type&&, decltype(*declval<InputIt>())>::value, int>::type = 0>
		ptr_vector(InputIt first, InputIt last, deleter_reference dt)
		...
		// assumes ownership of all pointers
		template<typename InputIt/*, typename std::enable_if<std::is_assignable<value_type, decltype(*declval<InputIt>())>::value, int>::type = 0*/>
	...
	private:
		...
		template <typename InputIt/*, typename std::enable_if<std::is_default_constructible<_Dx>::value && std::is_same<std::unique_ptr<T, _Dx>&&, decltype(*declval<InputIt>())>::value, int>::type = 0*/>
